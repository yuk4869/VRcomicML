...
PanelColumns = ['FIX_D_Min', 'FIX_D_Mean','FIX_D_Max', 'FIX_N',
...
for target in targets:
    ...
    for ID in range(2, 26, 1):
        ...
        described_data = Data_RemakePanel(described_data)
        Y_df = described_data.IsDifficult.astype('int')
        X_df = described_data
        try:
            X = X_df.reindex(columns = PanelColumns)
            X_train, X_test, Y_train, Y_test = train_test_split(X.drop('IsDifficult', axis =1), Y_df, test_size=0.3, stratify=Y_df, random_state=2434)
...
allResultAve.to_csv(path_or_buf = saveDir +"PersonalizedPanelResultAve.csv")
